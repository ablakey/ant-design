...
const AnchorContent = (props) => {
    ...
    process.env.NODE_ENV !== 'production'
      ? warning(
          !(
            ...
            (items === null || items === void 0 ? void 0 : items.some((n) => 'children' in n))
          ),
          'Anchor',
  ...
  const { direction, getTargetContainer } = React.useContext(ConfigContext);
  ...
  const updateInk = () => {
    ...
    const linkNode =
      (_a = wrapperRef.current) === null || _a === void 0
        ? void 0
        : _a.querySelector(`.${prefixCls}-link-title-active`);
  ...
  const getInternalCurrentAnchor = function (_links) {
    ...
    _links.forEach((link) => {
      const sharpLinkMatch = sharpMatcherRegex.exec(
  ...
  const createNestedLink = (options) =>
    Array.isArray(options)
      ...
      : null;
  const anchorContent = /*#__PURE__*/ React.createElement(
    'div',
    {
      ref: wrapperRef,
